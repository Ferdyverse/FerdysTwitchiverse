    ...
    async def async_wrapper(*args, **kwargs):
        try:
            ...
            if "NO_ACTIVE_DEVICE" in error_message:
                raise HTTPException(
                    status_code=400, detail="No active Spotify device found."
                )
            if e.http_status == 403:
                raise HTTPException(
                    status_code=403,
                    detail="Spotify command forbidden (e.g., playing restricted content).",
                )
            if e.http_status == 401:
                raise HTTPException(
                    status_code=401, detail="Unauthorized. Please log in again."
                )
