...
def check_access_rights(event, level: str):
    ...
    is_vip = (
        event._parsed["tags"].get("vip") == "1"
    )  # Check if user is a VIP (if available)
...
async def handle_command(bot, command_name: str, params: str, event):
    ...
    if callable(handler_function):
        logger.info(
            f"📢 Executing command: !{command_name} from {event.user.display_name}"
        )
...
async def command_commands(bot, params: str, event):
    ...
    try:
        ...
        await bot.send_message(
            f"@{event.user.display_name}, Ich konnte dir leider keine Nachricht senden!"
        )
...
async def command_addresponse(bot, params: str, event):
    ...
    if not check_access_rights(event, "mod"):
        await bot.send_message(
            f"⛔ @{event.user.display_name}, du besitzt nicht die notwendigen Rechte um Commands hinzuzufügen!"
        )
        logger.warning(
            f"⚠️ Unauthorized attempt: {event.user.display_name} tried to add a command."
        )
        return
    ...
    try:
        ...
        if command_name in COMMANDS:
            await bot.send_message(
                f"❌ Cannot override existing command function: !{command_name}"
            )
            return
...
async def command_todo(bot, params: str, event):
    ...
    if not check_access_rights(event, "mod"):
        await bot.send_message(
            f"⛔ @{event.user.display_name}, benutze die Kanalpunkte um ToDos hinzuzufügen"
        )
        logger.warning(
            f"⚠️ Unauthorized attempt: {event.user.display_name} tried to add a todo."
        )
        return

    if not params:
        await bot.send_message(
            f"❌ @{event.user.display_name}, bitte gib eine ToDo-Beschreibung an!"
        )
        return
...
async def command_hub(bot, params: str, event):
    ...
    if not params:
        await bot.send_message(
            f"⚠️ @{event.user.display_name}, bitte gib einen Text für den Hub an!"
        )
        return
...
async def command_tts(bot, params: str, event):
    ...
    if len(params) > 200:  # Limit text length
        await bot.send_message(
            f"⚠️ @{event.user.display_name} dein Text ist zu lang. Es sind maximal 200 Zeichen erlaubt!"
        )
        return
    ...
    if file_name:
        ...
        await bot.send_message(
            f"⚠️ @{event.user.display_name}, leider gab es ein Problem beim Generieren deiner Nachricht!"
        )
